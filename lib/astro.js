...
// ── Kullanıcıya ve güne göre değişen açılış cümleleri (uyum derecesine göre 3 grup, her grupta çeşitleme) ──
const DAY_MOOD_OPENERS = {
  high: [
    (natal, moon) => `${natal} burcu olarak bugün Ay'ın ${moon} burcunda olması sana iyi geldi, kendini rahat ve akışta hissettin.`,
    (natal, moon) => `Ay ${moon} burcundayken, ${natal} enerjinle uyum içindeydin — bugün işler senin için daha kolay ilerledi.`,
    (natal, moon) => `${natal} burcunun doğal ritmi bugün Ay'ın ${moon} burcuyla örtüştü, bu da içine bir rahatlık hissi getirdi.`,
    (natal, moon) => `Bugün gökyüzü sana gülümsedi: Ay ${moon} burcunda, tam da ${natal} burcunun sevdiği frekansta.`
  ],
  mid: [
    (natal, moon) => `${natal} burcu olarak, Ay'ın bugün ${moon} burcunda olması seni orta düzeyde etkiledi — ne çok kolay ne çok zordu.`,
    (natal, moon) => `Ay ${moon} burcundayken ${natal} enerjinle biraz gerilim biraz uyum yaşadın, dengeyi kendin kurman gerekti.`,
    (natal, moon) => `${natal} burcu için bugünün Ay'ı (${moon}) nötr bir alan açtı, seni ne çok zorladı ne de çok rahatlattı.`,
    (natal, moon) => `Bugün ${natal} burcunla Ay'ın ${moon} burcu arasında hafif bir gerilim vardı, ama yönetilebilir bir gündü.`
  ],
  low: [
    (natal, moon) => `${natal} burcu olarak bugün Ay'ın ${moon} burcunda olması seni biraz zorladı, sabrın test edilmiş olabilir.`,
    (natal, moon) => `Ay ${moon} burcundayken ${natal} enerjin biraz tıkandı — bugün kendine karşı nazik olmalıydın.`,
    (natal, moon) => `${natal} burcu için bugünün Ay'ı (${moon}) alışıldık ritmine ters düştü, belki biraz gergin hissettin.`,
    (natal, moon) => `Bugün ${natal} burcunla Ay'ın ${moon} burcu arasında görünmez bir sürtüşme vardı, ama bu da öğretici bir gündü.`
  ]
};

// ── Ay evresine göre ek yorum çeşitleri ──
const MOON_PHASE_CLAUSES = {
  'Yeni Ay':        ["Yeni Ay enerjisi yeni başlangıçlar için elverişliydi.", "Yeni Ay ile içine dönüp niyet belirlemek için iyi bir zamandı."],
  'Büyüyen Hilal':  ["Büyüyen Hilal, attığın adımları büyütmen için destekleyiciydi.", "Hilal büyürken senin de motivasyonun yavaşça artmış olabilir."],
  'İlk Dördün':     ["İlk Dördün, karar anlarını beraberinde getirdi.", "İlk Dördün enerjisiyle bugün küçük bir engelle karşılaşmış olabilirsin."],
  'Büyüyen Ay':     ["Büyüyen Ay, çabalarının meyvesini vermeye başladığı bir evreydi.", "Büyüyen Ay ile ilerleme kaydetmek için uygun bir gündü."],
  'Dolunay':        ["Dolunay duyguları yüzeye çıkardı, bugün her şey biraz daha yoğun hissettirmiş olabilir.", "Dolunay enerjisiyle farkındalıklar arttı, belki beklenmedik bir şey netleşti."],
  'Küçülen Ay':     ["Küçülen Ay, bırakma ve sadeleşme zamanıydı.", "Küçülen Ay ile bugün ihtiyacın olmayanı fark etmiş olabilirsin."],
  'Son Dördün':     ["Son Dördün, geride bıraktıklarınla yüzleşme fırsatı sundu.", "Son Dördün enerjisiyle bugün biraz düşünceye dalmış olabilirsin."],
  'Küçülen Hilal':  ["Küçülen Hilal, dinlenme ve içe dönme zamanıydı.", "Küçülen Hilal ile bugün enerjini toparlamak öncelikliydi."]
};

// ── Retro gezegen varsa eklenen not ──
const DAY_RETRO_CLAUSES = [
  planets => `Ayrıca ${planets} retrosunda olduğu için bugün bazı konular beklediğinden yavaş ilerlemiş olabilir.`,
  planets => `${planets} retro halindeyken, geçmişten bir konunun tekrar gündeme gelmesi şaşırtıcı olmaz.`,
  planets => `${planets} retrosu bugün iletişim ya da planlarında küçük aksaklıklara yol açmış olabilir.`
];

const DAY_MOOD_CLOSERS = [
  "Yıldızlar bir yön verir, seçim her zaman sende.",
  "Bugünün enerjisini nasıl kullandığın, tamamen senin elinde.",
  "Gökyüzü bir fon, hikâyeyi sen yazıyorsun.",
  "Bu bilgiyi bir pusula gibi düşün, harita değil.",
  "Yarın gökyüzü değişecek, bugünü değerlendir.",
  "Küçük bir farkındalık bile bugünü anlamlı kılabilir."
];

// Basit deterministik hash — aynı girdi her zaman aynı (ama girdi değişince farklı) sonucu üretir.
// Böylece aynı kullanıcı için gün boyu tutarlı, farklı kullanıcılar/günler için farklı metin seçilir.
...
/**
 * Bugünün (ya da geçmiş bir günün) gökyüzünün, kullanıcının burcuna göre "senin için ne anlama geldiği" yorumu.
 * Geriye dönük uyumluluk için eski çağrı biçimi de desteklenir: describeDayMood(moonSignName, natalSignName)
 */
function describeDayMood(opts, legacyNatalSignName) {
  if (typeof opts === 'string') {
    opts = { moonSign: opts, natalSun: legacyNatalSignName };
  }
  const {
    moonSign, natalSun, moonPhase, retrogradePlanets = [],
    seed = '', dateStr = new Date().toISOString().slice(0, 10)
  } = opts || {};

  if (!natalSun) {
    const mood = DAY_MOOD_BY_SIGN[moonSign] || 'değişkendi';
    return `Bu gün genel hava ${mood}.`;
  }

  const s1 = SIGNS.find(s => s.name === natalSun);
  const s2 = SIGNS.find(s => s.name === moonSign);
  const bucket = s1 && s2
    ? (elementCompatScore(s1.element, s2.element) >= 75 ? 'high' : elementCompatScore(s1.element, s2.element) >= 55 ? 'mid' : 'low')
    : 'mid';

  const baseSeed = `${dateStr}|${seed}|${natalSun}|${moonSign}`;
  const parts = [seededPick(DAY_MOOD_OPENERS[bucket], baseSeed + '|opener')(natalSun, moonSign)];

  if (moonPhase && MOON_PHASE_CLAUSES[moonPhase]) {
    parts.push(seededPick(MOON_PHASE_CLAUSES[moonPhase], baseSeed + '|phase'));
  }
  if (retrogradePlanets.length) {
    const planetsStr = retrogradePlanets.join(' ve ');
    parts.push(seededPick(DAY_RETRO_CLAUSES, baseSeed + '|retro')(planetsStr));
  }
  parts.push(seededPick(DAY_MOOD_CLOSERS, baseSeed + '|closer'));
